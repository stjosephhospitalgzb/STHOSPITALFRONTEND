...
const HOSPITAL = {
  ...
  opdTiming: "Monday – Saturday: 9:00 AM – 6:00 PM",
};
...
const STATS = [
  ...
  { icon:IC.user,   val:"400+",    label:"Expert Doctors"     },
                ...
                {FAQS.map((q,i) => (
                  <div key={i} className="hc-faq-item" style={{ borderBottom:i<FAQS.length-1?`1px solid ${C.border}`:"none", borderRadius:openFaq===i?10:0, overflow:"hidden", transition:"all .3s" }}>
                    <button onClick={() => setOpenFaq(openFaq===i?null:i)} style={{ width:"100%", display:"flex", alignItems:"center", justifyContent:"space-between", padding:"16px 12px", background:"none", border:"none", cursor:"pointer", fontFamily:"'Nunito',sans-serif", textAlign:"left" }}>
                      <span style={{ fontSize:14, fontWeight:700, color:C.dark }}>{q}</span>
                      <Ico d={openFaq===i?IC.chevU:IC.chevD} size={16} color={C.gray} />
                    </button>
                    {openFaq === i && (
                      <div style={{ padding:"0 12px 16px", fontSize:13, color:C.gray, lineHeight:1.7, animation:"fadeInUp .3s ease" }}>
                        Please contact our helpdesk at {HOSPITAL.phone} or email {HOSPITAL.emailAdmin} for detailed information about this query. Our team is available 24/7 to assist you.
                      </div>
                    )}
                  </div>
                ))}
              ...
              <Anim key={i} delay={i*0.09}>
                <div className="hc-stat-item" style={{ textAlign:"center", transition:"all .3s" }}>
                  <div style={{ width:54, height:54, borderRadius:"50%", background:"rgba(255,255,255,.12)", display:"flex", alignItems:"center", justifyContent:"center", margin:"0 auto 14px" }}>
                    <Ico d={s.icon} size={24} color={C.white} />
                  </div>
                  <div className="hc-stat-num" style={{ fontWeight:900, fontSize:"1.9rem", color:C.white, lineHeight:1, transition:"color .3s" }}>{s.val}</div>
                  <div style={{ fontSize:13, color:"rgba(255,255,255,.65)", fontWeight:600, marginTop:6 }}>{s.label}</div>
                </div>
              </Anim>
            ))}
          </div>
